			...
			if (#allfollows > 0) then
				for k,l in ipairs(allfollows) do
					if (issleep(l) == false) then
						...
						if (unit.strings[UNITTYPE] == "node") then
							name = "node"
						end
			...
			for a,unit in ipairs(ugroup[2]) do
				...
				if exists then
					...
					if (name ~= "empty") then
						...
						if (#thingshere > 0) then
							for a,b in ipairs(thingshere) do
								...
								if (thing.flags[CONVERTED] == false) and ((thingname == v) or ((thing.strings[UNITTYPE] == "text") and (v == "text")) or ((string.sub(thingname,1,6) == "glyph_") and (v == "glyph"))) then
									domake = false
								end
							end
						end
					...
					if domake then
						...
						elseif (v == "node") then
							if (name ~= "node") and (name ~= "all") then
								create("node_" .. name,x,y,dir,x,y,nil,nil,leveldata)
								updatecode = 1
							end
						elseif (v == "glyph") then
						...
						elseif (v == "event") then
							if (name ~= "event") and (name ~= "all") then
