			...
			if (getmat(thing) ~= nil) or (thing == "not " .. name) or (thing == "all") or (unitreference[thing] ~= nil) or ((thing == "text") and ((unitreference["text_text"] ~= nil or unitreference["node_text"] ~= nil) or (unitreference["glyph_metatext"] ~= nil))) or ((thing == "node") and ((unitreference["text_node"] ~= nil or unitreference["node_node"] ~= nil) or (unitreference["glyph_metanode"] ~= nil))) or (thing == "revert") or (thing == "createall") or (thing == "glyph") or (thing == "event") or ((operator == "write") and (getmat_text("text_" .. name) or getmat_text(name))) or ((operator == "inscribe") and (getmat("glyph_" .. name) or getmat(name))) or ((string.sub(name, 1, 5) == "text_") and getmat_text(name)) then
					...
					for a,b in ipairs(featureindex[name]) do
						...
						if (verb == "is") or (verb == "become") then
							if (target == name) and (object ~= "word") and (object ~= "break") and (object ~= "symbol") and ((object ~= name) or (verb == "become")) then
								if (object ~= "text") and (object ~= "glyph") and (object ~= "event") and (object ~= "node") and (object ~= "revert") and (object ~= "createall") then
				...
				if (#output > 0) then
					...
					for k,v3 in pairs(output) do
						...
						if (op == "is") then
							if (findnoun(object,nlist.brief) == false) and (object ~= "word") and (object ~= "break") and (object ~= "symbol") and (object ~= "text") and (object ~= "glyph") and (object ~= "event") and (object ~= "node") then
						...
						elseif (op == "write") or (op == "inscribe") then
							table.insert(conversions, v3)
						end
					end
